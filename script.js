...
const daftarMatkul = [
  ...
  {
    kode: "PTI05",
    nama: "Literasi Komputer",
    sks: 2,
    status: "Lulus",
    Dosen: "ibu Annisa",
    deskripsi: "Soeftware & Hardware"
  }
];
...
if (formLogin) {
  formLogin.addEventListener("submit", function(e) {
    ...
    if (nim === "250212080" && password === "123456789") {
...
counters.forEach(counter => {
    ...
    let interval = setInterval(() => {
        ...
        if(target % 1 !== 0){
            counter.innerHTML = angka.toFixed(2);
        } 
        else {
            counter.innerHTML = Math.floor(angka);
        }
    }, 30);
});